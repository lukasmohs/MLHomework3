...
lr2 <- optim(par=rep(0,2), fn=sq_loss, y=df$y, x1=df$x1, x2=df$x2)
summary(lr2)
```

c. Perform logistic regression with L2 regularization
```{r, quiet=T, message=F}
sq_loss_l2_regularized = function(pars, y, x, lambda=100) {
  sq_loss(pars,y,x) + lambda*sum(pars^2)
  # sq_loss(pars,y,x) + lambda*sum(pars[-1]^2) # often we omit the intercept term
}
```
