...
quadratic_term_prediction = function(x1,x2, pars) {
   pars[1] + x1* pars[2] + x2 * pars[3] + x1^2*pars[4] + x2^2 * pars[5] + x1*x2*pars[6]
}
sq_loss = function(pars, y, x1,x2) {
  sum((y-quadratic_term_prediction(x1,x2,pars))^2)
}
optquad <- optim(par=rep(0,6), fn=sq_loss, y=df$y, x1=df$x1, x2=df$x2)
optquad$par
...
optquadreg <- optim(par=rep(0,6), fn=sq_loss_l2_regularized, y=df$y, x1=df$x1, x2=df$x2)
optquadreg$par
```

f. Create a learning curve for parts b, c, d, e. Interpret your learning curve.
```{r, quiet=T, message=F}
#c("%","lr","opt","optreg","quadopt","quadoptreg")
...
for (i in 1:100 ){
  ...
  train = df[seq(1,nrow(df)/100*i,1),]
  opt <- optim(par=rep(0,3), fn=sq_loss, y=train$y, x1=train$x1, x2=train$x2)
  optreg = optim(par=rep(0,3), fn=sq_loss_l2_regularized, y=df$y, x1=df$x1, x2=df$x2)
  optquad <- optim(par=rep(0,6), fn=sq_loss, y=df$y, x1=df$x1, x2=df$x2)
  optquadreg <- optim(par=rep(0,6), fn=sq_loss_l2_regularized, y=df$y, x1=df$x1, x2=df$x2)
  for (c in 1:nrow(df)) {
    if((opt$par[1] + opt$par[1]*df$x1[c] + opt$par[2]*df$x2[c] > 0.5) == (df$y[c]==1 )) {
      optaccuracy = optaccuracy + 1
    }
    if((optreg$par[1] + optreg$par[1]*df$x1[c] + optreg$par[2]*df$x2[c] > 0.5) == (df$y[c]==1 )) {
      optregaccuracy = optregaccuracy + 1
    }
    if((optquad$par[1] + optquad$par[1]*df$x1[c] + optquad$par[2]*df$x2[c] > 0.5) == (df$y[c]==1 )) {
      optquadaccuracy = optquadaccuracy + 1
    }
    if((optquadreg$par[1] + optquadreg$par[1]*df$x1[c] + optquadreg$par[2]*df$x2[c] > 0.5) == (df$y[c]==1 )) {
      optquadregaccuracy = optquadregaccuracy + 1
    }
    accuracies$x[i] <- i
    accuracies$opt[i] <- optaccuracy
    accuracies$optreg[i] <- optregaccuracy
    accuracies$optquad[i] <- optquadaccuracy
    accuracies$optquadreg[i] <- optquadregaccuracy
  }
}
