...
RFPred <- prediction( rfPredictions[,2], statin_test$mi)
...
NNPredictedProbabilities = predict(NN,statin_test, type="prob") 
NNPred <- prediction( NNPredictions[,2], statin_test$mi)
NNROC <- performance(NNPred,"tpr","fpr")

#nnPredictions = data.frame(
 # nnpr = compute(nn,nnSpectTest %>% select(-V1))$net.result,
  #truth = spectTest$V1==1) %>% tbl_df()
#plot(nn)
#nnPredictions %>% mutate(nnpr=nnpr>0.5) %>% table()
